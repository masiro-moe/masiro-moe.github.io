self.$__dart_deferred_initializers__=self.$__dart_deferred_initializers__||Object.create(null)
$__dart_deferred_initializers__.current=function(a,b,c,$){var B={a_Q:function a_Q(){},Hq:function Hq(d,e){this.a=d
this.b=e},
aNS(d){switch(d.a){case 0:return D.aC
case 1:return D.aY}},
aWD(d,e,f,g,h,i){var x,w,v,u,t,s,r
if(e==null)return h
x=i.qA(e,0,h)
w=i.qA(e,1,h)
v=g.at
v.toString
u=x.a
t=w.a
if(u<t)s=Math.abs(v-u)<Math.abs(v-t)?x:w
else if(v>u)s=x
else{if(!(v<t)){v=i.c
v.toString
r=e.bO(0,y.i.a(v))
return A.jr(r,h==null?e.gk8():h)}s=w}g.xm(0,s.a,d,f)
return s.b},
b_g(d,e,f){var x,w
switch(e.a){case 0:x=d.R(y.o)
x.toString
w=B.aNS(x.w)
return w
case 1:return D.H}},
aWo(d){return new A.us(null,null,C.a91,d,null)},
aWp(d,e){var x,w=d.wE(y.b)
if(w==null)return!1
x=A.z1(d).lQ(d)
if(w.w.A(0,x))return w.r===e
return!1}},A,C,D
B=a.updateHolder(c[79],B)
A=c[0]
C=c[120]
D=c[2]
B.a_Q.prototype={
a_4(d){if(y.h.b(d))++d.fm$
return!1}}
B.Hq.prototype={
N(){return"ScrollViewKeyboardDismissBehavior."+this.b}}
var z=a.updateTypes([]);(function inheritance(){var x=a.inherit
x(B.a_Q,A.v)
x(B.Hq,A.dM)})()
A.bg(b.typeUniverse,JSON.parse('{"Hq":{"S":[]}}'))
var y={o:A.r("ef"),b:A.r("us"),i:A.r("p"),h:A.r("hS")};(function constants(){var x=a.makeConstList
C.fN=new B.Hq(0,"manual")
C.a8o=new B.Hq(1,"onDrag")
C.Wd=A.a(x([]),A.r("o<en>"))
C.a4B=new A.n(0,{},C.Wd,A.r("n<en,aV>"))
C.a91=new A.dd(C.a4B,A.r("dd<en>"))})()}
$__dart_deferred_initializers__["lG9lD6hkRL06CqLTQrNSuciqTGc="] = $__dart_deferred_initializers__.current
