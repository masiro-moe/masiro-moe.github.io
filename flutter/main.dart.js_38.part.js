self.$__dart_deferred_initializers__=self.$__dart_deferred_initializers__||Object.create(null)
$__dart_deferred_initializers__.current=function(a,b,c,$){var B={abX:function abX(){},
b4U(d){return B.aYA(d)},
b4T(d){return A.zW(d,new B.abY(),y.o)},
aYA(d){var x,w=J.aJ(d),v=A.an(w.h(d,"id")),u=A.an(w.h(d,"createdAt"))
if(u==null){u=A.h7(1970,1,1,0,0,0,0,!0)
if(!A.et(u))A.Q(A.e0(u))
u=new A.cj(u,!0)}else u=A.f3(u,!1)
x=A.an(w.h(d,"updatedAt"))
if(x==null){x=A.h7(1970,1,1,0,0,0,0,!0)
if(!A.et(x))A.Q(A.e0(x))
x=new A.cj(x,!0)}else x=A.f3(x,!1)
return new B.hg(v,u,x,A.b2(w.h(d,"title")),A.b2(w.h(d,"brief")),A.cu(w.h(d,"force")),A.an(w.h(d,"creator")),A.b2(w.h(d,"creatorName")),A.cu(w.h(d,"open")),A.an(w.h(d,"rank")),A.b2(w.h(d,"content")))},
hg:function hg(d,e,f,g,h,i,j,k,l,m,n){var _=this
_.a=d
_.b=e
_.c=f
_.d=g
_.e=h
_.f=i
_.r=j
_.w=k
_.x=l
_.y=m
_.z=n},
abY:function abY(){}},A,J
B=a.updateHolder(c[26],B)
A=c[0]
J=c[1]
B.abX.prototype={}
B.hg.prototype={
cz(){var x=this
return A.L(["id",x.a,"createdAt",x.b.a,"updatedAt",x.c.a,"title",x.d,"brief",x.e,"force",x.f,"creator",x.r,"creatorName",x.w,"open",x.x,"rank",x.y,"content",x.z],y.g,y.b)},
gh5(d){return this.z}}
var z=a.updateTypes(["hg(a5<f,@>)","bx<hg>(a5<f,@>)"])
B.abY.prototype={
$1(d){return B.aYA(d)},
$S:z+0};(function installTearOffs(){var x=a._static_1
x(B,"bhd","b4U",0)
x(B,"b_A","b4T",1)})();(function inheritance(){var x=a.inheritMany,w=a.inherit
x(A.u,[B.abX,B.hg])
w(B.abY,A.bd)})()
var y={o:A.C("hg"),g:A.C("f"),b:A.C("@")};(function lazyInitializers(){var x=a.lazyFinal
x($,"blX","b1b",()=>A.cH("/announcement/detail"))
x($,"blZ","aSO",()=>A.cH("/announcement/page"))
x($,"blY","aOK",()=>new B.abX())})()}
$__dart_deferred_initializers__["ocZzT0brnsbLI4pjmBpeWkYa4O0="] = $__dart_deferred_initializers__.current
